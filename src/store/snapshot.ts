...
// 深度克隆并移除不可序列化的数据（如函数），确保数据可以被 IndexedDB 存储
const serializeForIndexedDB = <T>(data: T): T => {
  ...
  actions: {
    ...
    async initSnapshotDatabase() {
      ...
      const newFirstSnapshot = {
        ...
        slides: serializeForIndexedDB(slidesStore.slides),
      }
    ...
    async addSnapshot() {
      ...
      if (this.snapshotCursor >= 0 && this.snapshotCursor < allKeys.length - 1) {
        needDeleteKeys = allKeys.slice(this.snapshotCursor + 1) as number[]
      }
      ...
      if (snapshotLength > snapshotLengthLimit) {
        needDeleteKeys.push(allKeys[0] as number)
        snapshotLength--
      }
